...
public class OpenAiPlayAnalyzer implements PlayAnalyzer {
    ...
    static final String PROMPT_TEMPLATE = """
            ...
            키워드는 다음 규칙을 모두 지켜 작성하세요.
            - 각각 한글 2~3자의 짧은 단어로 작성하세요.
            - 플레이 성향을 간결하게 표현하는 띄어쓰기 없는 명사 또는 형용사 형태로 작성하세요.
            - 문장이나 구절 형태로 작성하지 마세요.
            - 실제 게임 정보와 나이트 이동 경로에서 드러난 플레이 특성을 반영하세요.
            - 입력과 무관하게 동일한 일반 키워드만 반복적으로 선택하지 마세요.

            각인 이름은 다음 규칙을 모두 지켜 작성하세요.
            - 양자리, 사자자리 등 실제로 존재하는 별자리 이름은 사용하지 마세요.
            - 플레이 성향을 표현하는 창작 이름을 사용하세요.
            - '의 궤적' 앞부분은 한글 2~4자의 자연스러운 명사 또는 명사형 표현으로 작성하세요.
            - 앞부분은 하나의 의미로 자연스럽게 읽히는 표현을 사용하세요.
            - 서로 다른 단어를 억지로 결합한 표현이나 띄어쓰기가 포함된 구절은 사용하지 마세요.
            - 완성된 이름이 '의 궤적'과 자연스럽게 연결되도록 작성하세요.
            - 최종 이름은 반드시 '~의 궤적' 형식으로 작성하고 '의 궤적'으로 끝나게 하세요.
            - 입력 정보에 등장하는 기물명이나 게임 용어를 이름에 그대로 사용하지 마세요.
            - '나이트', '체스', '게임', '이동', '경로' 등의 단어를 이름에 직접 사용하지 마세요.
            - 선정한 플레이 성향 키워드를 바탕으로 이름을 창작하세요.
            - 플레이 성향 표현에 특정 단어를 금지어로 지정하지 마세요.
            - 입력과 무관한 일반적인 이름을 습관적으로 반복하지 마세요.
            - 게임 정보, 점수, 턴 수와 나이트 이동 경로에서 확인되는 특징을 이름에 반영하세요.
            - 서로 다른 게임 정보와 이동 경로에는 가능한 한 서로 다른 어휘와 표현을 사용하세요.

            코멘트는 다음 규칙을 모두 지켜 작성하세요.
            - 완료된 게임의 플레이를 돌아보며 플레이어에게 직접 이야기하는 자연스러운 과거형 존댓말 한 문장으로 작성하세요.
            - 반드시 '당신은'으로 시작하세요.
            - 현재형인 '~합니다.'로 끝내지 마세요.
            - '~했습니다.' 하나로만 제한하지 말고 자연스러운 과거형 존댓말로 끝내세요.
            - 공백과 문장부호를 포함해 최소 35자 이상, 최대 50자 이하로 작성하세요.
            - 게임 정보와 나이트 이동 경로를 바탕으로 플레이 성향을 자연스럽게 해석해 작성하세요.
            - 단순한 평가 한마디, 명사형 또는 지나치게 짧은 문장으로 작성하지 마세요.
