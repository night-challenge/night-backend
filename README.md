...
- 나이트 이동 궤적을 생성 로직으로 재구성해 별자리 디자인 생성
- AI가 플레이 성향을 분석해 각인 이름·키워드·코멘트 추천
- 생성된 좌표 데이터를 프론트엔드에서 SVG로 렌더링

> AI가 별자리 이미지를 직접 생성하는 구조가 아니라, 플레이 궤적을 별자리로 재구성하고 AI가 해당 디자인에 이름과 이야기를 더하는 방식입니다.
...
- 제품 코드 자동 생성
- 신청 상태별 목록 조회 및 신청 취소

**👤 마이페이지**
- 사용자 정보와 제품 각인 신청 여부 조회
- 가장 최근에 생성한 각인 카드 조회
- 보유 각인 카드 모음 조회
...
1. 게임 시작
2. 사용자 이동 및 AI 응수
3. 게임 결과와 나이트 이동 궤적 저장
4. 별자리 디자인 생성
5. AI 플레이 분석
6. 각인 이름 결정
7. 제품 및 각인 색상 선택
8. 제품 각인 신청
9. 마이페이지에서 신청 내역과 보유 카드 조회
...
> Railway 무료 Trial 크레딧으로 운영 중입니다. 크레딧 소진 또는 배포 인시던트로 일시적으로 접속이 안 될 수 있으니, 문제 발생 시 [로컬 실행 방법](#-로컬-실행-방법)으로 대체 확인 가능합니다.
